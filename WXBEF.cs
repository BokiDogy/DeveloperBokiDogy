...
namespace TrainLine.DAL
{
    public class WXBEF : OracleHelper
    {
        ...
        //存入多条记录
        protected int AddInfoList<T>(List<T> listoc, string tableName)
        {
            int count = 0;
            listoc.ForEach(t =>
            {
                count += InsertInfo(t, tableName);
            });
            return count;
        }
        //存入单条记录
        protected int InsertInfo<T>(T oc, string tableName)
        {
            Type pt = typeof(T);
            List<PropertyInfo> ptprs = pt.GetTypeInfo().DeclaredProperties.ToList();
            string colsql = "";
            ...
            else
            {
                foreach (PropertyInfo p in ptprs)
                {
                    ...
                    else
                    {
                        ...
                        colsql += "," + pname;
                        valsql += ",:" + pname;
                        paras.Add(new OracleParameter(":" + pname, p.GetValue(oc)));
                    }
                }
                colsql = colsql.Substring(0);
                valsql = valsql.Substring(0);
                string sql = $@"insert into {tableName} ({colsql}) values ({valsql})";
                int count = base.ExecuteNonSelect(sql, paras);
                return count;
            }
        }
    }
