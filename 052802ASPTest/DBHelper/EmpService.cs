...
namespace AspTest.DAl
{
    public class EmpService : OracleHelper
    {
        ...
        {
            string sql = "insert into test_0320emps (empno,ename,job,sal,deptno) values(:eno,:ena,:jo,:sal,:dno)";
            List<OracleParameter> paras = new List<OracleParameter>();
        ...
        {
            string sql = @"update test_0320emps e set ename=:en,job=:jo,sal=:sa,deptno=:dpn where empno=:eo";
            List<OracleParameter> paras = new List<OracleParameter>();
            paras.Add(new OracleParameter(":en", em.Ename));
            paras.Add(new OracleParameter(":jo", em.Job));
            paras.Add(new OracleParameter(":sa", em.Sal));
            paras.Add(new OracleParameter(":dpn", em.Deptno));
            paras.Add(new OracleParameter(":eo", em.Empno));
            int result = base.ExecuteNonSelect(sql, paras);
            return result > 0 ? true : false;
        }
